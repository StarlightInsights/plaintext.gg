...
// Clears the (multi-slug) IndexedDB and all web storage so each test starts
// clean. Uses the shared 'plaintext' DB under 'documents' store.
...
test.describe('Multi-document slug routing', () => {
  ...
  test('root path uses the "current" slug by default', async ({ page }) => {
    ...
    // Confirm IDB stored it under the 'current' key.
  ...
  test('invalid slug path returns 404', async ({ page }) => {
...
test.describe('Documents picker slug validation', () => {
  async function openDocumentsDialog(page) {
    await page.goto('/');
    await page.waitForSelector('#app-shell:not(.loading)');
    // Toolbar is hidden by default — surface it so the documents button is clickable.
    await page.evaluate(() => {
  ...
  test('reserved slug "current" shows an inline error and does not navigate', async ({ page }) => {
    ...
    // Still on root, not navigated to /current
